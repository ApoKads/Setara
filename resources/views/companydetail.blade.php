                ...
                <div class="h-18 overflow-visible">
                  <div class="w-36 h-36 bg-[#F2F6FF] rounded-full flex justify-center items-center relative -top-18 left-0">
                    ...
                    <div class="w-34 h-34 bg-blue-900 rounded-full flex items-center justify-center overflow-hidden">
                      @if($detail->path_logo)
                        <img src="{{ asset('storage/' . $detail->path_logo) }}" alt="{{ $detail->name }}" class="w-full h-full object-cover">
                      @else
            ...
            <div class="flex flex-wrap gap-2">
              @if($detail->categories && $detail->categories->count() > 0)
                @foreach($detail->categories as $category)
                  <span class="bg-{{ $category->color }}-100 text-{{ $category->color }}-800 text-sm font-semibold px-2.5 py-1 rounded-lg">{{ $category->name }}</span>
                @endforeach
              @else
          ...
          <div class="p-6">
            ...
            <div class="space-y-3">
              ...
              <div class="flex items-center gap-3">
                ...
                <div class="text-gray-800">
                  @if($detail->website)
                    <a href="{{ $detail->website }}" target="_blank" class="text-blue-600 hover:text-blue-800 underline">{{ $detail->website }}</a>
                  @else
                    Not Available
                  @endif
                </div>
              ...
              <div class="flex items-center gap-3">
                ...
                <div class="text-gray-800">
                  @if($detail->user && $detail->user->email)
                    <a href="mailto:{{ $detail->user->email }}" class="text-blue-600 hover:text-blue-800 underline">{{ $detail->user->email }}</a>
                  @elseif($detail->email)
                    <a href="mailto:{{ $detail->email }}" class="text-blue-600 hover:text-blue-800 underline">{{ $detail->email }}</a>
                  @else
                    Not Available
                  @endif
                </div>
                ...
                <div class="flex items-center gap-3 p-3 bg-white rounded-lg transition-colors mb-4">
                  ...
                  <div class="w-12 h-12 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden">
                    @if($company->path_logo)
                      <img src="{{ asset('storage/' . $company->path_logo) }}" alt="{{ $company->name }}" class="w-full h-full object-cover">
                    @else
